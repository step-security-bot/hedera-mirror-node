...
class ContractCallServiceERCTokenTest extends Web3IntegrationTest {
    ...
    void ercPrecompileOperationsTest(ContractFunctions ercFunction) {
        final var functionHash =
                functionEncoder.functionHashFor(ercFunction.name, CONTRACT_NAME, ercFunction.functionParameters);
        final var serviceParameters = serviceParameters(functionHash);
        final var successfulResponse =
                functionEncoder.encodedResultFor(ercFunction.name, CONTRACT_NAME, ercFunction.expectedResultFields);
    ...
    @Test
    void metadataOf() {
        final var functionHash = functionEncoder.functionHashFor("tokenURI", CONTRACT_NAME, NFT_ADDRESS, 1L);
        final var serviceParameters = serviceParameters(functionHash);
    ...
    private void persistEntities() {
        ...
        domainBuilder.nftAllowance().customize(a ->
                        a.tokenId(nftEntity.getId())
                                ...
                                .payerAccountId(senderEntityId))
                .persist();
        domainBuilder.tokenAccount().customize(a ->
                        a.balance(12)
                                .accountId(senderEntityId.getId())
                                .tokenId(fungibleTokenEntity.getId()))
                .persist();
    }
}
